...
```
tar -C $HOME/Desktop/Standard-OCP-Workshop/Lab1.2-Cluster_Installation_AWS/ -xvf $HOME/Desktop/openshift-install-mac-4.2.20.tar.gz
```
...
```
sed -e "s/<CLUSTER_DOMAIN>/${CLUSTER_DOMAIN}/g" -e "s/<CLUSTER_NAME>/${CLUSTER_NAME}/g" install-config-aws.yaml.template > install-config.yaml
```
